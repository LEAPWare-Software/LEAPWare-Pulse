...
directive 5 and `scripts/lwp_lanes.py`.

## When a review is required

Any commit (with an `LWP-Agent: claude` or `LWP-Agent: codex` trailer,
...
before `scripts/lwp_lanes.py` (the `lwp-lanes` CI job) passes. Bootstrap
exception: enforced only for PR numbers greater than 5 — see
`scripts/lwp_lanes.py`'s module docstring.
...
```json
{
  ...
  "verdict": "AGREE",
  "notes": "Checked core/lwp_core/rules/new_rule.py against the schema; agrees with the codex worker's read of directive 15."
}
```
...
- `reviewer_id` and `commit_author_id` are free-form identity strings (a
  ...
  reviewing itself). `scripts/lwp_lanes.py` enforces this by literal string
