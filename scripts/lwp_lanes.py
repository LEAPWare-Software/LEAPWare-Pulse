#!/usr/bin/env python3
"""Lane classification shared by the `lwp-lanes` CI check and the in-repo
session lane hooks (`scripts/lwp_check_lane_write.py`).

Owner directive (LWP-R8): "Codex works only on the Codex part, Claude only
on the Claude part. Shared parts may be changed by either CLI only after
the CTO/CIO role on each CLI adversarially checks and agrees; the owner is
not in that loop."

Lane membership (literal, per the plan's target layout):
...
the claude lane, and likewise `test_codex_*` / `*_codex_*` for codex --
the literal glob alone would strand a module like
`tests/adapters/test_claude_hook_io.py` in neither lane nor the shared
list.

Bootstrap exception: lane enforcement (this module's `check_lanes`, wired
into the `lwp-lanes` CI job) is a no-op for PR numbers 1-5 -- the PR that
...
TRAILER_RE = re.compile(r"^LWP-Agent:\s*(claude|codex|human)\s*$", re.MULTILINE)
...
def commit_agent(sha: str) -> Optional[str]:
    """The `LWP-Agent:` trailer value for `sha`, or None if missing/invalid."""
...
def check_lanes(rev_range: str, pr_number: int) -> list[str]:
    ...
    for sha in commits_in_range(rev_range):
        ...
        if agent is None:
            errors.append(f"{sha[:12]}: missing or invalid 'LWP-Agent:' trailer")
            continue
        if agent == "human":
        ...
        for f in files:
            ...
            if cls == "shared":
                ...
                errors.append(
                    f"{sha[:12]} (LWP-Agent: {agent}): touches '{f}', outside the "
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    if args.pr_number == 0:
        print("lwp-lanes check skipped (not running under a PR)")
    elif args.pr_number <= BOOTSTRAP_LAST_EXEMPT_PR:
        print(f"lwp-lanes check skipped (bootstrap exception, PR #{args.pr_number})")
    else:
        print("lwp-lanes check passed")
    return 0
