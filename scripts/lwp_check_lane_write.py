...
and `.codex/hooks.json` (see those files), separate from the lwp PRODUCT's
own policy hooks -- this one enforces owner directive (LWP-R8, "Codex
...
`scripts/lwp_lanes.py` uses for the commit-level `lwp-lanes` CI check. The
...
record (see module docstring); the `lwp-lanes` CI check still catches an
out-of-lane commit regardless of what this hook did or didn't catch.

Usage:
    python scripts/lwp_check_lane_write.py --agent claude
    python scripts/lwp_check_lane_write.py --agent codex
...
from lwp_lanes import REPO_ROOT, classify_path  # noqa: E402
        ...
        return {
            "hookSpecificOutput": {
                ...
                "permissionDecisionReason": (
                    f"lwp lane guard: {agent} may not write outside its lane or shared "
                    f"paths: {', '.join(offenders)} (see docs/architecture.md, LWP-R8)"
                ),
            }
        }
    return {
