#!/usr/bin/env python3
"""Validate and regenerate the generated block of HANDOFF.md.
...
Usage:
    python scripts/lwp_handoff.py --check                      # validate HANDOFF.md, exit 1 on failure
    python scripts/lwp_handoff.py --write [--cli NAME] [--session ID]
        ...
        # free-form strings and optional (default "unknown") -- this
        # script does not validate them against any external identity
        # source.
...
# Two caps, deliberately. LWP-R11 sets the requirement's hard cap at 6000
# bytes; the owner's own context-file ruling caps HANDOFF.md at 3000. The
# stricter one is what `--check` enforces -- 3000 satisfies R11's "at most
# 6000" by construction, and enforcing only 6000 would let the file drift
# to twice the size the owner asked for with nothing to catch it. Raising
# ENFORCED_CAP_BYTES back to 6000 is an owner decision, not a maintenance
# convenience. Use --cap to check a fixture against a different cap.
REQUIREMENT_CAP_BYTES = 6000  # LWP-R11
OWNER_CAP_BYTES = 3000  # owner ruling, 2026-09-17
SIZE_CAP_BYTES = OWNER_CAP_BYTES

def _rel(path: Path) -> str:
    """Repo-relative POSIX path, never absolute.

    This output is captured verbatim into proof records (proof/LWP-D*.json,
    `commands[].tail`). Printing an absolute path would plant a drive-letter
    or /home/<user> path in a committed file and trip LWP-R3's own env-leak
    check -- the proof of a clean tree would be the thing that dirties it.
    """
    try:
...
BEGIN_MARKER = "<!-- lwp-handoff:begin -->"
END_MARKER = "<!-- lwp-handoff:end -->"
...
# (no local-environment dependence, LWP-R3) and keeps a private identity
# out of a machine-read file.
FORBIDDEN_PATTERNS = [
...
# gitignored file (PRIVATE_NAMES_FILE, shared with scripts/lwp_check_env_leak.py)
...
def _proof_state_lines() -> list[str]:
    ...
    Reuses scripts/lwp_check_proof.py's own record validator so this never
    drifts from what the `lwp-proof` CI job itself enforces. An empty
    proof/ directory is not an error; it just means the list is "(none yet)".
    """
    import lwp_check_proof  # local import: only --write needs this, --check must not

    if not PROOF_DIR.is_dir():
        return ["(none yet)"]

    records = sorted(
        p for p in PROOF_DIR.glob("*.json") if p.name not in lwp_check_proof._SCHEMA_FILENAMES
    )
    if not records:
        return ["(none yet)"]
    ...
    for path in records:
        try:
            ...
            continue
        errors = lwp_check_proof._validate_record(path, data)
        is_gate = isinstance(data, dict) and "gate" in data
        label = data.get("gate", path.stem) if is_gate else data.get("deliverable", path.stem) if isinstance(data, dict) else path.stem
        if errors:
            lines.append(f"- {label}: INVALID ({errors[0]})")
        elif is_gate:
            lines.append(f"- {label}: PROVEN (gate record, verdict {data.get('verdict')!r})")
        else:
            lines.append(f"- {label}: PROVEN (commit {data.get('commit')})")
    return lines
...
def cmd_check() -> int:
    ...
    if errors:
        ...
        return 1
    print(f"OK: {_rel(HANDOFF_PATH)} passes all checks ({len(text.encode('utf-8'))} bytes)")
    return 0
...
def cmd_write(cli: str = "unknown", session: str = "unknown") -> int:
    ...
    if errors:
        ...
        return 1
    print(f"OK: wrote regenerated block to {_rel(HANDOFF_PATH)}")
    return 0
