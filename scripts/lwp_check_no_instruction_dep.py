#!/usr/bin/env python3
"""CI check `lwp-no-instruction-dep`: runtime code never names CLAUDE.md or
AGENTS.md.

Owner directive (LWP-R4): "no shipped runtime file under core/, adapters/,
plugins/** reads CLAUDE.md or AGENTS.md." Those files are contributor-only
docs (this repo's own `CLAUDE.md` / `AGENTS.md` at the root tell a human or
CLI which lane to work in) -- no hook, adapter, or engine module may read
them or even name them, or a future edit could quietly grow a real
dependency.
...
`core/` and `adapters/` do not exist until D1. Each scan root is skipped,
not an error, when absent -- so this check passes cleanly pre-D1.

Usage:
    python scripts/lwp_check_no_instruction_dep.py

Stdlib only. Exits 0 and prints "lwp-no-instruction-dep check passed" on
...
def main() -> int:
    ...
    if violations:
        for v in violations:
            print(f"FAIL: {v}")
        return 1
    print("lwp-no-instruction-dep check passed")
    return 0
