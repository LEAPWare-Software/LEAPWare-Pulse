# LEAPWare Pulse Private Release Design

## Goal

Prove that LEAPWare-Pulse is tested, independently vetted, privately released, and installed in Codex for authorized LEAPWare use.
...
4. **Runtime validation:** The exact reviewed plugin is installed through the supported Codex local-marketplace flow. A fresh Codex task verifies discovery and explicit `lw-pulse` use; an ordinary status request verifies automatic selection when evidence permits.
