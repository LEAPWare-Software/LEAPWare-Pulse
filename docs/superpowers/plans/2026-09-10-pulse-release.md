# LEAPWare Pulse Private Release Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Deliver a tested, independently vetted, privately released, and verified-installed LEAPWare-Pulse plugin.
...
**Spec:** `docs/superpowers/specs/2026-09-10-pulse-release-design.md`

## Global Constraints

- Keep `LEAPWare-Software/LEAPWare-Pulse` private and LEAPWare-only.
- Preserve the plugin name `LEAPWare-Pulse` and command phrase `lw-pulse`.
...
**Files:**
- Create: `scripts/validate_pulse_plugin.py`
- Create: `tests/test_validate_pulse_plugin.py`
...
Create `tests/test_validate_pulse_plugin.py` with `unittest` cases that import `validate`, confirm the real package passes, then copy the package to temporary directories and prove rejection of: a wrong manifest name, undeclared extra top-level capability, missing full-plan rule, missing unknown-denominator rule, incorrect percentage-floor rule, missing authority boundary, and missing `$LEAPWare-Pulse` default prompt.

- [ ] **Step 2: Run the tests and verify RED**

Run: `py -3.12 -m unittest discover -s tests -v`

Expected: FAIL because `scripts.validate_pulse_plugin` does not exist.

- [ ] **Step 3: Implement the minimal validator**

Create `scripts/validate_pulse_plugin.py` using only `argparse`, `json`, and `pathlib`. `validate()` must parse the manifest, inspect the exact package inventory, read `SKILL.md` and `agents/openai.yaml`, and return one error per violated acceptance invariant. The CLI prints every error and exits 1 when any exist; otherwise it prints `LEAPWare-Pulse validation passed` and exits 0.
...
Run both `py -3.12 -m unittest discover -s tests -v` and `py -3.12 scripts/validate_pulse_plugin.py plugins/LEAPWare-Pulse`.
...
**Files:**
- Modify only for a reproducible Task 2 failure: `plugins/LEAPWare-Pulse/skills/LEAPWare-Pulse/SKILL.md`
- Modify only if UI metadata becomes inconsistent: `plugins/LEAPWare-Pulse/skills/LEAPWare-Pulse/agents/openai.yaml`
...
**Files:**
- Modify: `plugins/LEAPWare-Pulse/.codex-plugin/plugin.json` through the cachebuster helper.
- Modify: `docs/validation.md`
- Create: `docs/evidence/release-2026-09-10.md`

**Interfaces:**
- Consumes: reviewed candidate, QA evidence, CI result, configured private marketplace `LEAPWare-Pulse`.
...
Run `update_plugin_cachebuster.py` against `plugins/LEAPWare-Pulse`. Do not hand-edit marketplace configuration.
...
Run `codex plugin add LEAPWare-Pulse@LEAPWare-Pulse`. Confirm installed and enabled status, and compare hashes for the manifest, skill, and UI metadata with private `main`.

- [ ] **Step 6: Validate in fresh Codex tasks**

Verify skill discovery, explicit `lw-pulse`, and ordinary status selection in fresh tasks. Save the raw results and limits; do not equate discovery with behavior.
