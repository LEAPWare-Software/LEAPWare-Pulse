...
delivery is announced as `LWP - Alert:`.**

## What goes here

One `proof/<deliverable-id>.json` per deliverable, validated against
`schema.json` by `scripts/lwp_check_proof.py` (the `lwp-proof` CI job).
...
`LWP-D0.json` and `LWP-D2.json`, were written for those deliverables'
...
`lwp-proof check passed` when every record is clean, or a distinct
