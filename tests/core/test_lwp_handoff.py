"""Unit tests for scripts/lwp_handoff.py. No network; git/gh calls are
...
SCRIPT_PATH = REPO_ROOT / "scripts" / "lwp_handoff.py"

_spec = importlib.util.spec_from_file_location("lwp_handoff", SCRIPT_PATH)
lwp_handoff = importlib.util.module_from_spec(_spec)
sys.modules["lwp_handoff"] = lwp_handoff
_spec.loader.exec_module(lwp_handoff)


def _valid_text(begin: str = lwp_handoff.BEGIN_MARKER, end: str = lwp_handoff.END_MARKER) -> str:
    return (
        "# HANDOFF\n\n"
        "## Start of session\n- [ ] read this\n\n"
        "## In flight\n1. do the thing\n\n"
...
def test_forbidden_substrings_default_only_when_file_absent(tmp_path, monkeypatch):
    monkeypatch.setattr(lwp_handoff, "REPO_ROOT", tmp_path)
    assert lwp_handoff._forbidden_substrings() == lwp_handoff.DEFAULT_FORBIDDEN_SUBSTRINGS


def test_forbidden_substring_from_local_file_fails(tmp_path, monkeypatch):
    (tmp_path / lwp_handoff.PRIVATE_NAMES_FILE).write_text("AdoptersSecretProject\n", encoding="utf-8")
    monkeypatch.setattr(lwp_handoff, "REPO_ROOT", tmp_path)
...
def test_cmd_check_missing_file(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr(lwp_handoff, "HANDOFF_PATH", tmp_path / "HANDOFF.md")
    assert lwp_handoff.cmd_check() == 1
...
def test_cmd_check_valid_file(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(lwp_handoff, "HANDOFF_PATH", path)
    assert lwp_handoff.cmd_check() == 0
...
def test_cmd_write_regenerates_only_the_block(tmp_path, monkeypatch):
    ...
    path.write_text(_valid_text(), encoding="utf-8")
    monkeypatch.setattr(lwp_handoff, "HANDOFF_PATH", path)
    monkeypatch.setattr(lwp_handoff, "_run_git", lambda args: "cafef00d")
    monkeypatch.setattr(lwp_handoff, "_run_gh", lambda args: "")

    before = path.read_text(encoding="utf-8")
    prose_before = before.split(lwp_handoff.BEGIN_MARKER)[0]

    assert lwp_handoff.cmd_write() == 0

    after = path.read_text(encoding="utf-8")
    prose_after = after.split(lwp_handoff.BEGIN_MARKER)[0]
    assert prose_before == prose_after
    assert "cafef00d" in after
    assert after.count(lwp_handoff.BEGIN_MARKER) == 1
    assert after.count(lwp_handoff.END_MARKER) == 1


def test_cmd_write_requires_existing_markers(tmp_path, monkeypatch, capsys):
    path = tmp_path / "HANDOFF.md"
    path.write_text(_valid_text().replace(lwp_handoff.BEGIN_MARKER, ""), encoding="utf-8")
    monkeypatch.setattr(lwp_handoff, "HANDOFF_PATH", path)
    assert lwp_handoff.cmd_write() == 1
...
def test_cmd_write_records_cli_and_session(tmp_path, monkeypatch):
    ...
    path.write_text(_valid_text(), encoding="utf-8")
    monkeypatch.setattr(lwp_handoff, "HANDOFF_PATH", path)
    monkeypatch.setattr(lwp_handoff, "PROOF_DIR", tmp_path / "no-such-proof-dir")
    monkeypatch.setattr(lwp_handoff, "_run_git", lambda args: "cafef00d")
    monkeypatch.setattr(lwp_handoff, "_run_gh", lambda args: "")

    assert lwp_handoff.cmd_write(cli="claude", session="sess-123") == 0
...
def test_cmd_write_lists_deliverable_proof_state(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lwp_handoff, "HANDOFF_PATH", path)
    monkeypatch.setattr(lwp_handoff, "PROOF_DIR", proof_dir)
    monkeypatch.setattr(lwp_handoff, "_run_git", lambda args: "cafef00d")
    monkeypatch.setattr(lwp_handoff, "_run_gh", lambda args: "")
...
def test_real_handoff_md_passes_check():
    ...
    assert lwp_handoff._validate(text) == []
